...
def add_indicators(df):
    ...
    indicator_bb = ta.volatility.BollingerBands(
        close=df["close"], window=5, window_dev=2
    )
...
def generate_trade(
    filtered_balances, orderbook, df_daily_recent, df_hourly_recent, fear_greed_index
):
...
def o1_generate(env):
    ...
    df_daily = pyupbit.get_ohlcv(
        "KRW-BTC", interval="day", count=44
    )  ## RSI 데이터 제공으로 인해 14일 추가하여 호출
    ...
    messages = generate_trade(
        filtered_balances, orderbook, df_daily, df_hourly, fear_greed_index
    )
